...
2. Configurar comando de instalacion y `pnpm build`.
...
6. Verificar rutas dinamicas, assets, 404 y robots en preview; el sitemap se genera solo en produccion.
...
| 7 | 7.1 Preparar Cloudflare Pages sin tocar producción | Preparada; despliegue pendiente de autorización | `.nvmrc`, `Docs/CLOUDFLARE_PAGES_PREVIEW.md`, `astro.config.mjs`, `package.json`, `pnpm-lock.yaml`, `wrangler.jsonc`, `Docs/RESULTADOS_ETAPA_7.md` | Revisión de configuración local, reproducción del fallo con `pnpm check`/`pnpm build`, corrección del nombre Wrangler, retirada del adaptador innecesario para salida estática y verificación posterior | Configurados el comando `pnpm build`, salida `dist`, Node `22.12.0` y el nombre Wrangler válido `print3x`. El build estático vuelve a generar `dist/index.html`; la preview y el proyecto Cloudflare siguen sin crearse y no se modificó DNS. |
...
| 7 | 7.2 (SEO) Automatizar sitemap con `@astrojs/sitemap` | Completada con preview de Cloudflare pendiente | `package.json`, `pnpm-lock.yaml`, `astro.config.mjs`, `src/pages/robots.txt.ts`, `src/pages/sitemap.xml.ts` (eliminado), `Docs/CLOUDFLARE_PAGES_PREVIEW.md`, `Docs/RESULTADOS_ETAPA_7.md`, `AGENTS.md` | `pnpm install --frozen-lockfile`, `pnpm check`, `pnpm build` en preview y produccion, inspeccion independiente de XML, robots, canonicals y noindex | Se fijo `@astrojs/sitemap@3.7.3` con allowlist exacta de 28 URLs. Produccion genera `sitemap-index.xml` y `sitemap-0.xml`; preview no genera sitemap. Se retiraron `Disallow` de rutas noindex en produccion para permitir que Google lea sus metadatos. No se modificaron DNS, produccion ni Shopify. |
| 7 | 7.2 (ajuste posterior del shell) Añadir enlace al repositorio de GitHub en el footer | Completada | `src/components/Footer.astro`, `Docs/RESULTADOS_ETAPA_7.md`, `Docs/ETAPAS_Y_TAREAS_MIGRACION.md` | `pnpm check`, `pnpm build`, `git diff --check`, inspeccion del HTML generado y preview local a `390px`, `768px`, `1024px` y `1440px` | GitHub queda como sexto icono social con el enlace solicitado; el SVG, el enlace accesible y la apertura segura funcionan en todos los viewports comprobados sin overflow. DNS y produccion no se modifican. |
| 6 | 6.3 Remediacion posterior: metadata social Open Graph | Completada | `src/layouts/SiteLayout.astro`, rutas de inicio, productos, colecciones, blogs, articulos, paginas y auxiliares, `Docs/RESULTADOS_ETAPA_6.md` | `pnpm check`, `pnpm build`, build de produccion con `PUBLIC_SITE_ENV=production`, inspeccion de 38 HTML, robots, sitemap, imagenes absolutas, fallback y rutas `noindex` | Todas las paginas generan `og:image`, `og:title`, `og:description` y Twitter Card; se usa la primera imagen aprobada por tipo de ruta o el logo local como fallback. Produccion genera robots y sitemap definitivos; las 8 rutas controladas conservan `noindex, nofollow`. DNS y produccion no se modifican. |
| 7 | 7.1 Retirar bloqueo SEO global de preview local | Completada; preview publica requiere proteccion de acceso | `src/layouts/SiteLayout.astro`, `src/pages/robots.txt.ts`, `astro.config.mjs`, `AGENTS.md`, `Docs/CLOUDFLARE_PAGES_PREVIEW.md`, `Docs/RESULTADOS_ETAPA_7.md` | `pnpm check`, `pnpm build`, inspeccion de robots, sitemap y robots meta | Se retiro `PUBLIC_SITE_ENV`; todos los builds generan sitemap y SEO publico para las rutas aprobadas. Se conservan solo las 8 exclusiones SEO intencionales. Una preview publica debe protegerse mediante Cloudflare Access u otro control de plataforma. DNS y produccion no se modifican. |
| 7 | 7.2 Crear `llms.txt` para agentes | Completada; validacion de hosting pendiente | `public/llms.txt`, `Docs/RESULTADOS_ETAPA_7.md`, `Docs/ETAPAS_Y_TAREAS_MIGRACION.md` | Lectura del documento indicado, especificacion `https://llmstxt.org`, propuesta Answer.AI, `pnpm check`, `pnpm build`, preview local de `/llms.txt`, estado de 25 enlaces y `git diff --check` | Se creo el archivo en `public/` para publicarlo como `/llms.txt`. Contiene 1 H1, resumen, contexto, 4 secciones H2 y 25 enlaces absolutos curados a contenido publico. Las rutas auxiliares, noindex, retiradas y contenido pendiente quedan fuera. La verificacion del tipo, cache y estado en Cloudflare Pages sigue pendiente; DNS y produccion no se modifican. |
| 7 | 7.2 Integrar Google Analytics despues de la carga | Completada; consentimiento legal pendiente | `src/layouts/SiteLayout.astro`, `Docs/RESULTADOS_ETAPA_7.md`, `Docs/ETAPAS_Y_TAREAS_MIGRACION.md` | `pnpm install --frozen-lockfile`, `pnpm check`, `pnpm build`, inspeccion de HTML generado, preview local, red y consola | Se incorporo la propiedad GA4 `G-F5XTDB1B1S` mediante `gtag.js`. El script externo se crea dinamicamente despues de `load` y se programa en idle, por lo que no participa en la carga critica. No se incorporo `GTM-5VNX4MX`; DNS y produccion no se modifican. |
| 7 | 7.2 Integrar Microsoft Clarity despues de la carga | Completada; consentimiento legal pendiente | `src/layouts/SiteLayout.astro`, `Docs/RESULTADOS_ETAPA_7.md`, `Docs/ETAPAS_Y_TAREAS_MIGRACION.md` | `pnpm check`, `pnpm build`, inspeccion de HTML generado, preview local, red, proyecto `y95dr45vtg`, consola y `git diff --check` | Se incorporo Clarity con carga dinamica despues de `load` y en periodo idle. La cola y la guarda son independientes de GA4; no existe etiqueta remota estatica. DNS y produccion no se modifican. |
| 7 | 7.2 Integrar transicion suave de acordeones `details` | Completada; compatibilidad parcial documentada | `src/styles/components/accordion.css`, `Docs/RESULTADOS_ETAPA_7.md`, `Docs/ETAPAS_Y_TAREAS_MIGRACION.md` | Consulta de Chrome/MDN sobre `::details-content`, `interpolate-size` y `transition-behavior`; `pnpm check`; `pnpm build`; preview local de producto; apertura/cierre; comprobacion de menu y `git diff --check` | Los acordeones `.accordion details` animan `block-size` y `content-visibility` con deteccion de soporte, `overflow: clip` y respeto de `prefers-reduced-motion`. Los menus `details` del header permanecen fuera del selector. Los navegadores sin soporte mantienen el comportamiento nativo; DNS y produccion no se modifican. |
| 7 | 7.2 Integrar transicion de slices del hero del index | Completada | `src/pages/index.astro`, `src/scripts/print3x-ui.ts`, `src/scripts/README.md`, `src/styles/components/homepage.css`, `Docs/RESULTADOS_ETAPA_7.md`, `Docs/ETAPAS_Y_TAREAS_MIGRACION.md` | `pnpm check`, `pnpm build`, preview local en `390px`, `768px`, `1024px` y `1440px`, click de siguiente, navegacion por teclado, `prefers-reduced-motion`, overflow, consola y `git diff --check` | El hero usa una transicion opt-in donde la nueva slice entra de derecha a izquierda sobre la anterior. El estado se limpia al terminar, teclado y movimiento reducido mantienen el cambio instantaneo, y no se modifica el comportamiento de otros sliders. DNS y produccion no se modifican. |
| 7 | 7.2 Correccion de salida de slices del hero del index | Completada | `src/scripts/print3x-ui.ts`, `src/scripts/README.md`, `src/styles/components/homepage.css`, `Docs/RESULTADOS_ETAPA_7.md`, `Docs/ETAPAS_Y_TAREAS_MIGRACION.md` | `pnpm check`, `pnpm build`, preview local, inspeccion geometrica de slide y `.homepage__slide-box` durante la salida, estados finales, overflow, consola y `git diff --check` | La slide anterior se desplaza a `translateX(-100%)` junto con su recuadro y queda bajo la nueva mediante un nivel de apilamiento propio; ya no permanece visible sobre el hero. DNS y produccion no se modifican. |
